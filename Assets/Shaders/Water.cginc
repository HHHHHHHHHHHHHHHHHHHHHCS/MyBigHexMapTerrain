...
float Foam(float shore, float2 worldXZ, sampler2D noiseTex)
{
	...
	float4 noise = tex2D(noiseTex,frac( noiseUV * (2*TILING_SCALE)));//这里3随便用一个平铺比例
...
float Waves(float2 worldXZ, sampler2D noiseTex)
{
	float2 uv1 = worldXZ;
	uv1.y += _Time.y;
	float4 noise1 = tex2D(noiseTex,frac( uv1 *  (3*TILING_SCALE)));//这里3随便用一个平铺比例
	
	float2 uv2 = worldXZ;
	uv2.x += _Time.y;
	float4 noise2 = tex2D(noiseTex,frac( uv2 *  (3*TILING_SCALE)));//这里3随便用一个平铺比例
